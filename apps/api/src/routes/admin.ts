...
// Create Hackathon
(router as any).post('/hackathons', requireAdmin, upload.single('poster'), async (req: any, res: Response) => {
  try {
    const { 
      name, description, startDate, endDate, location, city, 
      mode, eligibilityType, eligibleCollegesList, websiteUrl, imageUrl, tags 
    } = req.body;
    ...
    if (typeof eligibleCollegesList === 'string') {
      try {
        ...
        parsedColleges = eligibleCollegesList.split(',').map((c: string) => c.trim()).filter(Boolean);
      }
...
// Update Hackathon
(router as any).put('/hackathons/:id', requireAdmin, async (req: Request, res: Response) => {
  try {
...
// Delete Hackathon
(router as any).delete('/hackathons/:id', requireAdmin, async (req: Request, res: Response) => {
  try {
    const id = req.params.id as string;
...
(router as any).get('/users', requireAdmin, async (req: Request, res: Response) => {
  try {
...
// Delete User (Admin)
(router as any).delete('/users/:id', requireAdmin, async (req: Request, res: Response) => {
  try {
    const id = req.params.id as string;
...
(router as any).post('/host-requests', async (req: any, res: Response) => {
  try {
...
(router as any).get('/my-request', async (req: any, res: Response) => {
  try {
    const session = await auth.api.getSession({ headers: req.headers });
    if (!session) return res.status(401).json({ message: 'Unauthorized' });
...
(router as any).get('/host-requests', requireAdmin, async (req: Request, res: Response) => {
  try {
...
(router as any).put('/host-requests/:id', requireAdmin, async (req: Request, res: Response) => {
  try {
    const id = req.params.id as string;
